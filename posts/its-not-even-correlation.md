...
Now if we only look at successful people, the graph looks like this:
...
In this case, we illustrate that the amount of time spent doing sport or reading can not discriminate if you are successful or not. And the only way to know that, is to compare populations and seek for differences instead of similarities. 
